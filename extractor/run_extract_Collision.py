...
def main():
    ...
    for part_index in range(len(parts)):
        ...
        end_offset = parts[part_index + 1] - 1 if part_index + 1 < len(parts) else len(file_bytes) - 1
        try:
            export_collision(file_bytes, output_directory, start_offset, end_offset)
...
def export_collision(file_bytes: bytearray, output_directory: str, start_offset: int, end_offset: int):
    log_file = join(output_directory, f"collision_export_log_{hex(start_offset)}.txt")
    if os.path.exists(log_file):
        os.remove(log_file)
    
    try:
        print(f"Start: {hex(start_offset)}, End: {hex(end_offset)}")
        collision = Collision(file_bytes, start_offset, end_offset)
        save_to_json(collision.to_dict(), Path(output_directory) / f"{hex(start_offset)}.json")
        print(f"    Successfully processed collision data at offsets {hex(start_offset)}-{hex(end_offset)}")    
    except Exception as e:
        log_to_file(log_file, f"Error: {str(e)}")
        print(f"    Error processing collision data at offsets {hex(start_offset)}-{hex(end_offset)}")
