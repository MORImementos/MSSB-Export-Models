...
def print_directory_structure(root_dir, indent_level=0, exclude_items=None):
    if exclude_items is None:
        exclude_items = [".pyc,", "__pycache__", "test"]
    ...
    for item in os.listdir(root_dir):
        ...
        #         if item_path.endswith(i):
        #             continue
        #         else:
        #             print(' ' * indent_level + '|-- ' + item)
        # else:
        #     print(' ' * indent_level + '|-- ' + item)
        if any(item_path.endswith(exclude) for exclude in exclude_items) or any(exclude in item_path for exclude in exclude_items):
            continue
        
        print(' ' * indent_level + '|-- ' + item)

        if os.path.isdir(item_path):
            if item_path.startswith(r"extractor\outputs") or item_path.startswith(r"extractor\data\test") or item_path.endswith("__pycache__"):
                continue
            else:
